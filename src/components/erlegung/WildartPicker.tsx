...
function getWildArtLabel(wildArt: WildArt): string {
  for (const details of Object.values(WILD_GROUP_DETAILS)) {
    if (!details) continue
    for (const list of [details.altersklassen_maennlich, details.altersklassen_weiblich, details.altersklassen_unbekannt]) {
...
export function WildartPicker({
  ...
  const handleGroupTap = useCallback((config: WildGroupConfig) => {
    ...
    if (config.hasGeschlecht) {
      ...
      setSelectedWildArt(config.unspezValue)
      setStep('detail')
  ...
  // --- Detail: Altersklasse ---
  const handleAltersklasseSelect = useCallback((wildArt: WildArt) => {
  ...
  const flatTiere = selectedGroup
    ...
    : undefined
  const altersklassen = selectedGeschlecht === 'maennlich'
    ? groupDetails?.altersklassen_maennlich
    : selectedGeschlecht === 'weiblich'
      ? groupDetails?.altersklassen_weiblich
      : null
        ...
        <div style={{ padding: '0 1rem 1.5rem' }}>
          ...
          {step === 'detail' && groupDetails && (
            <div style={{ marginTop: '0.25rem' }}>
                ...
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    {groupDetails.geschlechter.map(g => (
                      <button
                        key={g.value}
                        onClick={() => handleGeschlechtSelect(g.value)}
                        style={{
                          flex: 1,
                          padding: '0.625rem',
                          borderRadius: 'var(--radius)',
                          border: selectedGeschlecht === g.value
                            ? '2px solid var(--green)'
                            : '1px solid var(--border)',
                          background: selectedGeschlecht === g.value
                            ? 'var(--surface-3)'
                            : 'var(--surface-2)',
                          color: selectedGeschlecht === g.value
                            ? 'var(--text)'
                            : 'var(--text-2)',
                          cursor: 'pointer',
                          fontSize: '0.875rem',
                          fontWeight: selectedGeschlecht === g.value ? 600 : 400,
                          minHeight: '2.75rem',
              ...
              {/* Altersklasse (erst nach Geschlecht-Wahl) */}
              {altersklassen && altersklassen.length > 0 && (
                <div style={{ marginBottom: '1rem' }}>
                  <div style={{
                    fontSize: '0.6875rem',
                    color: 'var(--text-3)',
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                    {altersklassen.map(a => (
                      <button
                        key={a.value}
                        onClick={() => handleAltersklasseSelect(a.value)}
                        style={{
                          padding: '0.5rem 1rem',
                          borderRadius: 'var(--radius)',
