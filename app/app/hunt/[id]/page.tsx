...
type Hunt = { id: string; name: string; type: string; status: string; invite_code: string; wild_presets: string[]; started_at: string; signal_mode: string }
type Participant = { id: string; user_id: string | null; guest_name: string | null; role: string; tags: string[]; status: string; profiles: { display_name: string } | null }
        ...
        <div className="flex gap-1.5 px-3 py-2 overflow-x-auto flex-shrink-0"
          ...
          {participants.map((p, i) => (
            <div key={p.id} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full flex-shrink-0"
              ...
              <span className="text-xs font-medium">{p.role === 'jagdleiter' ? 'Du' : pName(p).split(' ')[0]}</span>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6">
        {activeTab === 'karte' && (
          <div className="text-center">
            ...
            <p className="text-lg font-bold mb-1">Live-Karte</p>
            <p className="text-sm" style={{ color: 'var(--text-3)' }}>
              Hier kommt die Leaflet-Karte mit<br />GPS-Positionen aller Teilnehmer.
            </p>
            <p className="text-xs mt-4 px-6 py-2 rounded-lg" style={{ background: 'var(--surface-2)', color: 'var(--text-3)' }}>
              Nächster Schritt: Supabase Realtime + Leaflet
            </p>
          </div>
        )}
        {activeTab === 'chat' && (
